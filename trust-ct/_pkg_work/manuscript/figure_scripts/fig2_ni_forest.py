...
Standalone fix (2026-08-29): dual-layout path resolution; heading-overlap fix.
"""
import os
import pathlib
import numpy as np
...
# Archive path map: processed/<prefix> -> results/<dir>
_ARCHIVE_MAP = [
...
NI_CSV = _locate("cimas", "results_phase3_freeze", "ni_client_level.csv")

# -- Load from locked CSV (no hardcoded values) --------------------------------
ni = pd.read_csv(NI_CSV)

methods  = ni["method"].tolist()
delta    = ni["obs"].values       # bootstrap NI point estimate
...
# Header placed below plot title to avoid overlap (y = len(methods) - 0.60)
header_x = 0.0035
ax.text(header_x, len(methods) - 0.60, r"$\Delta$AUROC [95% CI]",
