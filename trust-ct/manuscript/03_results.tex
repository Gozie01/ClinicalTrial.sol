...
The seed-averaged absolute minimum provider AUROC (lowest-performing provider
in each condition, marginal, not paired) was 0.518 [0.444, 0.578] under cold
FedAvg, 0.509 [0.466, 0.566] under cold local, and 0.525 [0.487, 0.565] under
warm-local (Figure~6B).
The identity of the lowest-performing provider varied across conditions and seeds;
these marginal minima do not constitute a paired contrast and their difference
($+0.009$) is not the worst-provider delta reported below.
...
The worst-provider \emph{paired} contrast---defined as the mean across five seeds of
the minimum per-provider AUROC difference (cold-FedAvg minus cold-local) across
$K=23$ providers---was $-0.114$ [$-0.141$, $-0.089$]; $p=0.001$.
This quantity identifies, within each seed, the provider most harmed by global
aggregation relative to local-only training; it is not the arithmetic difference
between the marginal absolute minima above, which may refer to different providers
